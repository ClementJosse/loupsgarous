<template>
    ...
    <div class="flex flex-row gap-[clamp(0px,3vw,15px)]">
    ...
    <button class="text-2xl text-blue-background font-semibold bg-purple-important py-[clamp(0px,1.5vw,7.5px)] px-[clamp(0px,20vw,100px)] rounded-full mt-[clamp(0px,30vw,150px)]">
        Créer une partie
    </button>
    ...
    <div v-if="lobbyGames.length > 0" >
        ...
        <ul class="flex flex-col justify-center justify-items-center">
          <button v-wave type="button" class= "bg-dark-background justify-center justify-items-center mb-[clamp(0px,3vw,15px)] py-[clamp(0px,1vw,5px)] w-[clamp(0px,84vw,420px)] rounded-lg" v-for="(game, index) in lobbyGames" :key="index">
